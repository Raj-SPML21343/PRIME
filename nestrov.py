...
def APGD_old(fx, gx, gradf, proxg, params):
    ...
    for k in range(maxit):
        ...
        x_k_next = y_k_next + ((k)/(k+3)) * (y_k_next - y_k)
        # z_k_next = proxg(z_k - alpha * ((k+1)/2) * gradf(x_k), alpha)
        # x_k_next = y_k_next * ((k+1)/(k+3)) + (2/(k+3)) * z_k_next
        # z_k = z_k_next
        y_k = y_k_next
        x_k = x_k_next
        info['fx'][k] = fx(x_k) + lmbd * gx(x_k)

    print("APGD OLD : Time Taken : {}".format(time.time() - time_start))
    return x_k, info
...
if __name__ == "__main__":
    ...
    reconstruction_l1 = optimize_L1(image, indices, APGD_old, params)[0]
    ...
    reconstruction_tv = optimize_TV(image, indices, APGD_old, params)[0]
